...
/**
 * 一次问题的 Agent 执行状态。
 * originalGoal 只在构造时写入，刻意没有 setter，避免检索候选或后续改写覆盖用户目标。
 */
public final class AgentExecutionState {
    ...
    public AgentExecutionState(String originalGoal) {
        ...
        if (normalized.isEmpty()) throw new IllegalArgumentException("originalGoal must not be blank");
    ...
    public Set<String> getCapabilityCallFingerprints() { return Collections.unmodifiableSet(capabilityCallFingerprints); }
    public String getLastProgressHash() { return lastProgressHash; }
    public boolean markProgress(String progressHash) {
        if (progressHash == null || progressHash.isBlank()) return false;
    ...
    public void stop(AgentStopReason stopReason) { this.stopReason = Objects.requireNonNull(stopReason, "stopReason"); }
    public boolean isStopped() { return stopReason != null; }
    public EvidenceCoverage getEvidenceCoverage() { return evidenceCoverage; }
    public void setEvidenceCoverage(EvidenceCoverage evidenceCoverage) { this.evidenceCoverage = Objects.requireNonNull(evidenceCoverage); }
}
