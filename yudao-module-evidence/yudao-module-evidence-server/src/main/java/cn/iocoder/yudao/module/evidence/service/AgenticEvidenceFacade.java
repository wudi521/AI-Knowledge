...
public class AgenticEvidenceFacade {
    ...
    public EvidenceEvaluateRespVO evaluateUnrecorded(String query, List<Long> kbIds, String domainCode,
        ...
        resp.setTraceId(traceId); resp.setQuery(query); resp.setHistory(safeHistory);
        resp.setConsultable(false); resp.setConflicts(List.of()); resp.setExecutionMode("AGENTIC_V1");
        resp.setIntent("AGENTIC_V1"); resp.setReasonCode(result.stopReason() == null ? null : result.stopReason().name());
        ...
        resp.setAnswerable(answer); resp.setAnswer(answer ? result.answer() : null);
        ...
        List<EvidenceEvaluateRespVO.EvidenceItemVO> evidenceItems = evidences.stream()
                .map(e -> toEvidence(e, kbIds.get(0), domainCode)).collect(java.util.stream.Collectors.toCollection(ArrayList::new));
        if (answer && evidenceItems.isEmpty()) evidenceItems.add(structuredEvidence(result.answer(), kbIds.get(0), domainCode));
    ...
    public void record(EvidenceEvaluateRespVO resp) {
        ...
        if (resp.getEvidence() != null) {
            for (EvidenceEvaluateRespVO.EvidenceItemVO item : resp.getEvidence()) {
                ...
                evidences.add(Evidence.builder().chunkId(item.getChunkId()).content(item.getContent())
                        .documentId(item.getDocumentId() == null ? null : String.valueOf(item.getDocumentId()))
                        .documentName(item.getDocumentName()).versionNo(item.getVersionNo()).versionId(item.getVersionId())
                        .score(item.getScore()).products(List.of())
                        .channels(item.getChannels() == null ? new ArrayList<>() : new ArrayList<>(item.getChannels()))
                        .chunkMetadata(item.getChunkMetadata()).build());
            }
        }
    ...
    private void applyGeneration(EvidenceEvaluateRespVO resp, GenerationResult generation) {
        ...
        if (generation.getClaims() != null) resp.setClaims(generation.getClaims().stream().map(this::toClaim).toList());
    }
    ...
    private EvidenceEvaluateRespVO.ClaimVO toClaim(ClaimResult claim) {
        ...
        vo.setText(claim.getText()); vo.setVerdict(claim.getVerdict()); vo.setEvidenceIndex(claim.getEvidenceIndex());
        return vo;
    }

    private EvidenceEvaluateRespVO stopped(String query, String traceId, String reasonCode, String reason, long start) {
        EvidenceEvaluateRespVO resp = new EvidenceEvaluateRespVO();
        resp.setTraceId(traceId); resp.setQuery(query); resp.setAnswerable(false); resp.setConfidence(null);
        resp.setConsultable(false); resp.setRefusalReason(reason); resp.setRoute("ABSTAIN"); resp.setIntent("AGENTIC_V1");
        resp.setExecutionMode("AGENTIC_V1"); resp.setReasonCode(reasonCode); resp.setEvidence(List.of());
        resp.setConflicts(List.of()); resp.setClaimFail(false); resp.setStages(List.of());
        resp.setElapsedMs((int) (System.currentTimeMillis() - start));
        return resp;
    }
    ...
    private EvidenceEvaluateRespVO.EvidenceItemVO toEvidence(Evidence evidence, Long kbId, String domainCode) {
        ...
        vo.setEvidenceId(evidence.getChunkId()); vo.setChunkId(evidence.getChunkId()); vo.setContent(evidence.getContent());
        vo.setChunkMetadata(evidence.getChunkMetadata()); vo.setDocumentName(evidence.getDocumentName());
        vo.setVersionNo(evidence.getVersionNo()); vo.setVersionId(evidence.getVersionId());
        vo.setDocumentId(parseLong(evidence.getDocumentId())); vo.setKbId(kbId); vo.setDomainCode(domainCode);
        vo.setScore(evidence.getScore()); vo.setEvidenceType("CHUNK");
    ...
    private EvidenceEvaluateRespVO.EvidenceItemVO structuredEvidence(String answer, Long kbId, String domainCode) {
        EvidenceEvaluateRespVO.EvidenceItemVO vo = new EvidenceEvaluateRespVO.EvidenceItemVO();
        vo.setEvidenceType("STRUCTURED_RESULT"); vo.setKbId(kbId); vo.setDomainCode(domainCode);
        vo.setContent(answer); vo.setScore(1D); vo.setFilters("agenticV1=true,evidenceCoverage=FULL");
        return vo;
    }
    ...
    private QueryStageTimingDTO stage(AgentTraceStep step) {
        ...
        dto.setStage("AGENT_" + step.phase()); dto.setSeq(step.seq()); dto.setStatus(step.status()); dto.setSkipped(false);
        dto.setElapsedMs(step.elapsedMs()); dto.setErrorCode(step.stopReason() == null ? null : step.stopReason().name());
        dto.setInputSummary(StrUtil.maxLength("action=" + StrUtil.nullToEmpty(step.action())
    ...
    private void fillMetadata(EvidenceEvaluateRespVO.EvidenceItemVO vo, String metadata) {
        ...
        try {
            ...
            vo.setApplicationNo(obj.getStr("applicationNo")); vo.setPublicationNo(obj.getStr("publicationNo"));
            vo.setSectionType(obj.getStr("sectionType")); vo.setSectionTitle(obj.getStr("sectionTitle"));
            vo.setClaimNo(obj.getStr("claimNo")); vo.setPageStart(obj.getInt("pageStart")); vo.setPageEnd(obj.getInt("pageEnd"));
    ...
    private String newTraceId() { return "ag-" + UUID.randomUUID().toString().replace("-", "").substring(0, 12); }
}
