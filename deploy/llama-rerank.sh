...
nohup llama-server \
  ...
  --port 1236 --reranking > /tmp/llama-rerank.log 2>&1 &
echo "llama-server(bge-reranker) 已启动, 端口 1236; 验证: curl -X POST http://127.0.0.1:1236/v1/rerank -H 'Content-Type: application/json' -d '{\"query\":\"x\",\"documents\":[\"a\",\"b\"]}'"
