...
## 四、端到端实证(2026-08-21 实测)

**问题**: 用户反馈"提示词还是写死的"——排查发现根因是**消费方服务旧进程用旧 Feign 路径**:
上午 10:09-10:16 日志大量 `PromptSupport: [get][prompt key(xxx) 获取失败, 回退内置默认: 请求参数缺失:id]`。

**根因**: model-server 的 RPC 路径在 commit 3bf04a2 从 `/prompt/get` 改名为 `/prompt/get-prompt`,
但当时 running 的 retrieval/evidence 服务未重启, 仍按旧路径调用 → 每次 400 → 静默回退代码默认。
(10:16 后四个服务重启完毕, 12:38 起日志零失败。)

**验证(改 DB → 行为变化)**: 把 DB `search-answer` 内容改为"只回答两个字: 收到",
等 30s 缓存过期后触发真实检索 → **answer 返回"收到"** → 改回原内容 → answer 恢复。
结论: 提示词管理**端到端真实生效**, DB 优先, 代码默认仅兜底。

**教训**: 消费方服务(Feign 路径/新依赖)改动后必须重启, 否则静默回退默认造成"改 DB 无效"假象;
排查此类问题先看消费方日志有没有 `PromptSupport ... 获取失败` 回退告警。
